...
const Hero: React.FC<HeroProps> = ({ isDark }) => {
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col-reverse lg:flex-row items-center gap-16 lg:gap-24">
          ...
          <div ref={textContainerRef} className="flex-1 text-center lg:text-left">
            <div className={`inline-flex items-center gap-2 mb-6 px-3 py-1 rounded-full border text-[11px] font-bold tracking-widest uppercase backdrop-blur-sm ${isDark ? 'border-neon-teal/20 bg-neon-teal/5 text-neon-teal' : 'border-blue-600/10 bg-blue-50 text-blue-700'
              }`}>
      ...
      ${isDark
                    ? 'bg-white text-black hover:bg-teal-300'
              ...
              <div className="flex items-center gap-8">
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className={`hero-action-item flex items-center gap-2 font-bold tracking-widest text-[11px] transition-all duration-300 ${isDark ? 'text-gray-400 hover:text-white' : 'text-slate-500 hover:text-slate-900'
                    }`}
                >
                  <Linkedin size={18} />
                  LINKEDIN
          ...
          <div ref={imageContainerRef} className="flex-1 relative flex justify-center lg:justify-end">
            <div className="relative w-64 h-64 md:w-80 md:h-80 lg:w-96 lg:h-96">
              ...
              <div className={`relative w-full h-full rounded-full p-2 border transition-all duration-700 overflow-hidden backdrop-blur-sm ${isDark ? 'border-white/10 bg-white/5' : 'border-slate-200 bg-white shadow-2xl'
                }`}>
                <img
                  ref={imageRef}
              ...
              <div className={`absolute -bottom-2 -right-2 px-6 py-3 rounded-full border backdrop-blur-xl shadow-2xl transition-transform duration-500 hover:scale-105 ${isDark ? 'bg-black/80 border-white/20 text-neon-teal shadow-neon-teal/10' : 'bg-white/95 border-slate-200 text-blue-700'
                }`}>
